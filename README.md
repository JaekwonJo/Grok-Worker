...
- 레퍼런스 이미지를 확장 프로그램 안에 미리 저장
- 프롬프트 안의 `@S999`, `@S998` 같은 토큰을 읽어서
  - 같은 이름의 이미지를 현재 프롬프트에 자동 첨부
...
- 사이드패널 UI 추가
- 레퍼런스 이미지 라이브러리 IndexedDB 저장 구조 추가
...
- `001`은 작업 번호
- `@S999`, `@S998`는 확장 프로그램 레퍼런스 이미지 이름
- 실제 입력창에는 `@`가 제거된 본문이 들어감
- 이미지는 현재 프롬프트에 자동 첨부됨
